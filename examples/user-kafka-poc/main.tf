...
# Grafana service
# TODO: the provider docs should be updated to provide docs for `aiven_service`
#   - i.e. want to understand inputs `cloud_name` `cloud_name`, etc?
# ref: https://registry.terraform.io/providers/aiven/aiven/latest/docs
resource "aiven_service" "samplegrafana" {
  project      = aiven_project.sample.project
  cloud_name   = "google-europe-west1" # TODO: what are valid option/regions/names here?
  plan         = "startup-4" # do we need this--is this the plan.out file or? 
  # hmm yes apparently its required but not sure what the requirements are for name and relationsip/deps, etc?
  # `Error: 400: {"errors":[{"message":"Invalid input: 'plan' is a required property","status":400}],"message":"Invalid input: 'plan' is a required property"} -`
